 ...
 * @since 0.3.1
 */
public final class ArticleCommentReplyNotifier extends
	...
	public void action(final Event<JSONObject> event) throws EventException {
		...
		if (Strings.isEmptyOrNull(originalCommentId)) {
			LOGGER.log(Level.DEBUG, "This comment[id={0}] is not a reply",
		...
		final CommentRepository commentRepository = beanManager
		...
		try {
			...
			final JSONObject replyNotificationTemplate = preferenceQueryService
					.getReplyNotificationTemplate();
		
			final String articleTitle = article
					.getString(Article.ARTICLE_TITLE);
			final String articleLink = Latkes.getServePath()
